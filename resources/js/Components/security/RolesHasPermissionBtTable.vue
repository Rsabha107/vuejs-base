...
function permissionsFormatter(value) {
    ...
    return value
        .map((p) => `<span class="perm-badge">${p.name}</span>`)
        .join("");
}
...
function initTable() {
    ...
    $table.bootstrapTable({
        ...
        columns: [
            { field: "id",          title: "ID",          sortable: true,  align: "center", width: "80px",  visible: false, switchable: true },
            { field: "name",        title: "Name",        sortable: true,  switchable: true },
            { field: "permissions", title: "Permissions", sortable: false, searchable: false, switchable: true, formatter: permissionsFormatter },
            { field: "actions",     title: "Actions",     sortable: false, searchable: false, switchable: false, align: "center", width: "120px", formatter: actionFormatter },
        ],
    });
...
async function openEditModal(id, name) {
    ...
    const { data } = await axios.get(`/api/roles-permissions?limit=1&offset=0&search=${encodeURIComponent(name)}`);
...
<template>
    <div class="bt-outer">
        ...
        <div id="rp-left-toolbar" style="display:block">
            <button class="bt-pill-btn" :disabled="isLoadingModal" @click="openCreateModal">
                <span v-if="isLoadingModal" class="spinner-border spinner-border-sm me-1"></span>
            ...
            <div class="rp-modal-box">
                ...
                <div class="rp-modal-header">
                    <h5 class="rp-modal-title">
                        ...
                        {{ modalMode === "create" ? "Assign Permissions to Role" : "Edit Role Permissions" }}
                    </h5>
                    ...
                    <div>
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <label class="form-label fw-semibold mb-0">
                                Permissions
                                <span class="badge-count">{{ form.permission_ids.length }} selected</span>
                            </label>
                            <button
                                type="button"
                        ...
                        <div class="perm-list">
                            ...
                            <div v-if="filteredPermissions.length === 0" class="text-muted small py-3 text-center">
                ...
                <div class="rp-modal-footer">
                    ...
                    <button class="btn btn-primary" :disabled="isSaving" @click="saveAssignment">
...
:deep(.fixed-table-toolbar .search input.form-control::placeholder) { color: rgba(255,255,255,0.65); }
...
:deep(.fixed-table-toolbar .btn.dropdown-toggle::after) { display: none; }
...
:deep(.fixed-table-toolbar .dropdown-menu .form-check-input) { border-radius: 4px; border-color: #5b8df6; }
:deep(.fixed-table-toolbar .dropdown-menu .form-check-input:checked) { background-color: #5b8df6; border-color: #5b8df6; }
...
:deep(.sortable .both),
:deep(.sortable .asc),
:deep(.sortable .desc) { color: #8899bb; }

:deep(table) { margin-bottom: 0 !important; }
...
:deep(tbody tr:hover td) { background: #f5f7ff !important; }
...
:deep(.fixed-table-pagination .pagination .page-item:hover:not(.active) .page-link) { background: #eaecf6; }
...
:deep(.bt-edit-btn)       { background: #fff7e6; border-color: #fde7b0; color: #d97706; }
:deep(.bt-edit-btn:hover) { background: #fef3c7; }

:deep(.bt-delete-btn)       { background: #fff1f2; border-color: #fecdd3; color: #dc2626; }
...
.perm-check-item:hover { background: #f5f7ff; }
...
.perm-checkbox:checked { background-color: #3a5bd9; border-color: #3a5bd9; }
