...
import VenueFormModal from "./VenueFormModal.vue";
...
const items = ref([
  { label: "Venues", url: "/venues", icon: "pi pi-map-marker" },
]);
...
async function confirmDelete({ id, title }) {
  ...
  try {
    await axios.delete(route("venues.destroy", id));
    window.toastr?.success("Venue deleted successfully");
    refreshTable();
  } catch (_) {
    window.toastr?.error("Failed to delete venue.");
  }
}
...
<template>
  ...
  <div class="bt-outer">
    <div id="venues-left-toolbar" style="display: block">
      <button class="bt-pill-btn" @click="openCreateModal">
        <i class="fa fa-plus me-1"></i> Add Venue
      </button>
    </div>
