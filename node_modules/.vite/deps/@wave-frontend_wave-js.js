...
//#region node_modules/@wave-frontend/wave-js/dist/index.js
var require_dist = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	((WaveAttributes2) => {
		...
		WaveAttributes2.component = "wave-component";
	...
	((WaveMessages2) => {
		...
		WaveMessages2.invalidComponentParams = "Invalid component parameters";
	})(WaveMessages || (WaveMessages = {}));
	var WaveDom = class {
		constructor(selector) {
			const element = document.querySelector(selector);
			if (!element) throw new Error(WaveMessages.elementNotFound.replace("%s", selector));
			this.parent = element;
		}
		get root() {
			return this.parent;
		...
		function parseCondition(condition, data) {
			...
			for (let i = 0; i < conditions.length; i++) {
				...
				for (let j = 0; j < operators.length; j++) {
					...
					if (!condition2.includes(logicalOperator)) continue;
					const args = condition2.split(logicalOperator);
					if (args.length < 2) continue;
					conditionMet = handler(WaveParser2.parseArgument(args[0], data), WaveParser2.parseArgument(args[1], data));
				}
				if (!conditionMet) {
					conditionsMet = false;
					break;
	...
	var WaveApp = class {
		...
		useStore(store) {
			if (this.dom) throw new Error(WaveMessages.mountedUseStore);
			this.stores.push(store);
		}
		useStores(...stores) {
			for (let i = 0; i < stores.length; i++) this.useStore(stores[i]);
		}
		mount() {
			if (this.dom) throw new Error(WaveMessages.alreadyMounted);
			this.dom = new WaveDom(this.selector);
			for (let i = 0; i < this.stores.length; i++) this.stores[i].addDataChangeCallback(this.onDataChange.bind(this));
			this.initializeMountedElement();
		}
		unmount() {
			if (!this.dom) throw new Error(WaveMessages.notMounted);
			const data = this.getMergedStoreData();
			const keys = Object.keys(data);
			for (let i = 0; i < keys.length; i++) {
				const key = keys[i];
				const elements = this.dom.getElementsByAttribute(WaveAttributes.data, key);
				for (let j = 0; j < elements.length; j++) elements[j].outerHTML = `{{ ${key} }}`;
			}
			...
			for (let i = 0; i < statements.length; i++) {
				...
				switch (statement.type) {
					...
					case "component":
						...
						const componentPromise = new Promise(async (resolve, reject) => {
							if (!this.dom) return reject(WaveMessages.notMounted);
							...
							if (tempElement) tempElement.outerHTML = content;
							resolve();
						});
		...
		onDataChange(instance, changedKey) {
			...
			for (let i = 0; i < elements.length; i++) elements[i].innerHTML = value.toString();
			this.updateConditionals();
		}
		updateConditionals() {
			if (!this.dom) return;
		...
		getMergedStoreData() {
			...
			for (let i = 0; i < this.stores.length; i++) {
				...
				for (let j = 0; j < keys.length; j++) {
					const key = keys[j];
					if (data[key]) console.warn(WaveMessages.storeKeyOverlap, key, this);
					data[key] = store.getValue(key);
				}
			}
			return data;
		}
		getMergedStoreComponents() {
			const components = {};
			for (let i = 0; i < this.stores.length; i++) {
				const store = this.stores[i];
				...
				for (let j = 0; j < keys.length; j++) {
					const key = keys[j];
					if (components[key]) console.warn(WaveMessages.storeKeyOverlap, key, this);
	...
	var WaveStore = class {
		...
		async setValue(key, value) {
			...
			if (proxy) {
				if (!await proxy(value)) return;
			}
			this.data[key] = value;
			for (let i = 0; i < this.dataChangeCallbacks.length; i++) this.dataChangeCallbacks[i](this, key);
		}
		getValue(key) {
		...
		get(key) {
			...
			return {
				...
				update: async (_value) => {
					return await this.set(key, _value);
				}
			};
		}
		async set(key, value) {
		...
		getComponentHandler(key) {
			return this.components[key];
		}
		setComponentHandler(key, handler) {
...
//# sourceMappingURL=@wave-frontend_wave-js.js.map